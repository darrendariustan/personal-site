...
export default function ChatWidget() {
  ...
  const [messages, setMessages] = useState([
    { role: "assistant", content: "Hi! I'm Darren's digital twin. Ask me anything about my career, skills, or experience." }
  ]);
  ...
  const sendMessage = async (e) => {
    ...
    try {
      ...
      if (response.ok) {
        ...
        setMessages((prev) => [...prev, { role: "assistant", content: `Error: ${data.error || "Oops, something went wrong on my end."}` }]);
      }
    } catch (error) {
      ...
      <AnimatePresence>
        {isOpen && (
          <motion.div
            ...
            className="fixed bottom-6 right-6 w-[350px] sm:w-[400px] h-[500px] glass-card rounded-2xl flex flex-col overflow-hidden z-50 border border-slate-700/50 shadow-2xl bg-slate-900/80"
          >
            {/* Header */}
